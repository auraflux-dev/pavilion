...
const DOCS = [
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'body',  text: 'SHMS PTO Platform — Wix Admin Guide  |  Last updated: July 22, 2026' },
      { type: 'h1',   text: 'Where to Go' },
      { type: 'body',  text: 'Day-to-day: Staff → Programs (registration, featured, schedule, flyer, roster, sessions, class messages).' },
      { type: 'body',  text: 'First-time create: Wix Content Manager → Programs, then finish schedule/flyer in Staff → Programs.' },
      { type: 'h1',   text: 'What parents see on /programs' },
      { type: 'body',  text: 'Name, price, grades, capacity, flyer image (or Flyer coming soon), and a schedule line: day · time · weeks · start–end dates.' },
      { type: 'body',  text: 'Visitors see the catalog; Register requires a free account (/auth/join). Paid membership is not required for enrichment registration.' },
      { type: 'h1',   text: 'Staff → Programs tabs' },
      { type: 'body',  text: 'Programs — open/close registration (VP Programs / admin only), featured, day of week, class time, weeks, start/end dates, upload flyer.' },
      { type: 'body',  text: 'Sessions — concrete meeting dates for the member portal calendar after enrollment.' },
      { type: 'body',  text: 'Roster — enrolled + waitlisted families, promote/cancel, CSV export, Message class (portal inbox).' },
      { type: 'h1',   text: 'Capacity and waitlist' },
      { type: 'body',  text: 'When capacity is full, new registrations are Waitlisted with a position number. Parents see waitlist confirmation. VP Programs promotes from the Roster tab when a seat opens.' },
      { type: 'body',  text: 'If a paid registration lands on the waitlist, staff must handle refund or promotion manually — check Roster feePaid / transactionId.' },
      { type: 'h1',   text: 'Schedule fields (CMS + Staff)' },
      { type: 'body',  text: 'dayOfWeek — e.g. Tuesdays' },
      { type: 'body',  text: 'classTime — e.g. 3:30–4:30 PM' },
      { type: 'body',  text: 'durationWeeks — number of weeks' },
      { type: 'body',  text: 'startDate / endDate — first and last meeting dates' },
      { type: 'body',  text: 'image — flyer URL (upload from Staff → Programs)' },
      { type: 'body',  text: 'Admin: Staff → Programs → Ensure CMS fields once if new fields are missing.' },
      { type: 'h1',   text: 'Instructors and coordinators' },
      { type: 'body',  text: 'Assign role instructor or coordinator on Staff → Staff access, plus Assigned program IDs (copy ID from Programs list).' },
      { type: 'body',  text: 'They only see assigned programs. They can edit schedule/flyer and use Roster/Sessions/Timesheets. They cannot open/close registration or set Featured.' },
      { type: 'h1',   text: 'When Changes Go Live' },
      { type: 'body',  text: 'Usually within a few minutes after save.' },
      { type: 'h1',   text: 'Quick Link' },
      { type: 'body',  text: 'Programs CMS: https://manage.wix.com/dashboard/509fda24-8dbf-43c6-aa74-df9f8b63c388/cms/Programs' },
    ],
  },
  {
    folder: 'wix',
    update: true,
    title: '02b - Contractor Timesheets (Instructors & Coordinators)',
    content: [
      { type: 'title', text: 'Contractor Timesheets (Instructors & Coordinators)' },
      { type: 'body',  text: 'SHMS PTO — Staff Guide  |  Last updated: July 22, 2026' },
      { type: 'h1',   text: 'Who uses this' },
      { type: 'body',  text: 'Instructors and coordinators (@shmspto.org + StaffRoles) submit hours for assigned enrichment programs.' },
      { type: 'body',  text: 'VP Programs and admin review and Approve / Reject under Staff → Timesheets.' },
      { type: 'h1',   text: 'How to submit hours' },
      { type: 'body',  text: '1. Sign in with your @shmspto.org account → /staff → Timesheets.' },
      { type: 'body',  text: '2. Choose your assigned program, work date, start time, end time, optional notes.' },
      { type: 'body',  text: '3. Click Submit to VP Programs. Status becomes Submitted.' },
      { type: 'h1',   text: 'How VP Programs reviews' },
      { type: 'body',  text: '1. Staff → Timesheets shows all contractor submissions.' },
      { type: 'body',  text: '2. Approve or Reject each Submitted row. Approved hours are ready for your payment process outside the site.' },
      { type: 'h1',   text: 'Setup checklist' },
      { type: 'body',  text: '1. Admin: Staff → Programs → Ensure CMS fields (creates ContractorTimesheets if needed).' },
      { type: 'body',  text: '2. Staff → Staff access: role instructor or coordinator + assigned program IDs.' },
      { type: 'body',  text: '3. Contractor must use @shmspto.org login (aliases OK for inbox forwarding).' },
    ],
  },
  {
    folder: 'wix',
    update: true,
    title: '02c - Parent Enrichment Registration (Free & Paid Members)',
    content: [
      { type: 'title', text: 'Parent Enrichment Registration (Free & Paid Members)' },
      { type: 'body',  text: 'SHMS PTO — Parent / Board Support Guide  |  Last updated: July 22, 2026' },
      { type: 'h1',   text: 'Who can register' },
      { type: 'body',  text: 'Any parent with a free site account can register for enrichment. Paid Lagoon/Ruby/etc. membership is not required for programs.' },
      { type: 'h1',   text: 'Visitor path' },
      { type: 'body',  text: '1. Browse /programs — see name, price, day/time/weeks/dates, flyer.' },
      { type: 'body',  text: '2. Tap Register → /auth/join (create account) → return to programs.' },
      { type: 'h1',   text: 'Logged-in parent path' },
      { type: 'body',  text: '1. Add student in Member Portal with phone, emergency contact, and authorized pick-up.' },
      { type: 'body',  text: '2. Open /programs → Register → choose student → accept consents.' },
      { type: 'body',  text: '3. Free programs enroll immediately. Paid programs continue to Square checkout.' },
      { type: 'body',  text: '4. If the class is full, you are waitlisted (with position). Staff will contact you if a seat opens.' },
      { type: 'h1',   text: 'After enrollment' },
      { type: 'body',  text: 'Member Portal calendar shows program schedule / session dates. Instructor messages appear in the portal inbox.' },
      { type: 'h1',   text: 'Board help' },
      { type: 'body',  text: 'If registration fails: confirm student safety fields, registration is open, and capacity/waitlist status under Staff → Programs → Roster.' },
    ],
  },
  {
    folder: 'wix',
    ...
    content: [
      ...
      { type: 'body',  text: 'SHMS PTO Platform — Wix Admin Guide  |  Last updated: July 17, 2026' },
      { type: 'h1',   text: 'Where to Go' },
      { type: 'body',  text: 'Wix Dashboard → Content Manager → Membership Tiers' },
      { type: 'h1',   text: 'What This Controls' },
      { type: 'body',  text: 'The membership cards on /membership — prices, tier names, perks lists, store-card credit, and the faculty membership section. Checkout uses productId on each paid tier (or Site Settings fallbacks).' },
      { type: 'h1',   text: 'Fields Explained' },
      { type: 'body',  text: 'tierId — Internal slug used by checkout and the portal. Paid examples: ruby | supreme | pearl. Faculty: faculty. Do not change an existing tierId after people have purchased it.' },
      { type: 'body',  text: 'name — Display name (e.g. "Pearl Membership")' },
      { type: 'body',  text: 'price — Display price in dollars (number only, no $ sign). Also set the same price on the Wix Stores product.' },
      ...
      { type: 'body',  text: 'giftCardCredit — Dollars loaded onto the student Square store card when this tier is purchased (e.g. 10 / 25 / 50). Set 0 to skip.' },
      { type: 'body',  text: 'productId — Wix Catalog product UUID for checkout. Required for any new paid tier. variantId optional.' },
      { type: 'body',  text: 'highlighted / popular — Check to show as "Most Popular"' },
      { type: 'body',  text: 'sortOrder — Lower numbers appear first' },
      { type: 'body',  text: 'active — Uncheck to hide a tier' },
      { type: 'h1',   text: 'How to Add a New Paid Tier (e.g. Pearl)' },
      { type: 'body',  text: '1. Wix Stores → Products → create and publish the membership product → copy Product ID' },
      { type: 'body',  text: '2. Content Manager → Membership Tiers → + New Item' },
      { type: 'body',  text: '3. Set tierId (slug), name, price, description, perks, giftCardCredit, productId, active = true, sortOrder' },
      { type: 'body',  text: '4. Optional Site Settings fallbacks: membershipPearlProductId, membershipPearlVariantId, membershipPearlGiftCardCredit, membershipPearlSlug' },
      { type: 'body',  text: '5. Wait ~1 minute, hard-refresh /membership, then test Join as a logged-in free parent' },
      { type: 'h1',   text: 'How to Update Prices / Perks / Store-card credit' },
      { type: 'body',  text: '1. Content Manager → Membership Tiers → open the tier' },
      { type: 'body',  text: '2. Change price, perks, and/or giftCardCredit → Save' },
      { type: 'body',  text: '3. Also update the Wix Stores product price so checkout matches the card' },
      { type: 'h1',   text: 'What happens after purchase' },
      { type: 'body',  text: 'Free or already-logged-in parents: Join → Wix checkout → return → "I\'ve completed payment". The site updates Students.membershipTier, Memberships, and creates/loads a Square gift card for giftCardCredit. Portal shows paid status.' },
      ...
      { type: 'body',  text: 'The faculty price and description come from the row with tierId = faculty. It does NOT appear in the main tier grid — only in the Faculty & Staff section lower on the page. Faculty is not a Wix checkout product.' },
      { type: 'h1',   text: 'When Changes Go Live' },
      { type: 'body',  text: 'Within about 1 minute (membership tiers cache is short).' },
      { type: 'h1',   text: 'Quick Link' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'body',  text: 'SHMS PTO Platform — Wix Admin Guide  |  Last updated: July 17, 2026' },
      { type: 'h1',   text: 'Where to Go' },
      { type: 'body',  text: 'Recommended view: Staff top nav → Events — shows upcoming events and a Manage in Wix Events button.' },
      { type: 'body',  text: 'Create or edit events: Wix Dashboard → Events (left sidebar — NOT Content Manager).' },
      { type: 'h1',   text: 'What This Controls' },
      { type: 'body',  text: 'The "Upcoming Events" section on the homepage and the full /events page.' },
      { type: 'h2',   text: 'Important' },
      { type: 'body',  text: 'Events use the native Wix Events app — they are NOT in Content Manager like other collections. Staff → Events is a read view + shortcut; create still happens in the Wix Events app.' },
      { type: 'h1',   text: 'How to Add an Event' },
      { type: 'body',  text: '1. From Staff → Events, click Manage in Wix Events (or go to Wix Dashboard → Events → "+ Create Event").' },
  ...
  // ── Store docs ───────────────────────────────────────────────────────────
  {
    ...
    content: [
      ...
      { type: 'h1', text: 'Needs Reconciliation (Staff → Payments)' },
      { type: 'body', text: 'Treasurer / admin: Staff top nav → Payments. Filter or open rows with status Needs Reconciliation.' },
      { type: 'body', text: 'Needs Reconciliation means Square payment completed but the gift-card LOAD failed. Verify the Square payment, then use Retry gift-card load or Mark reconciled — do not ask the parent to retry.' },
      { type: 'h1', text: 'Treasurer records (CMS backup)' },
  ...
  // ── New July 2026 editor guides (portal + PageContent era) ─────────────────
  {
    ...
    content: [
      ...
      { type: 'body',  text: 'Recommended: Staff top nav → Page copy.' },
      { type: 'body',  text: 'Backup / legacy: Wix Dashboard → Content Manager → Page Content' },
      ...
      { type: 'h1',   text: 'Steps' },
      { type: 'body',  text: '1. Preferred: Staff top nav → Page copy → open the page slug you need.' },
      { type: 'body',  text: '2. Or: Content Manager → Page Content → open the row whose page matches the site URL (e.g. programs for /programs, or home-volunteer for the home volunteer block)' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'h1',   text: 'Where to edit' },
      { type: 'body',  text: 'Recommended: Staff top nav → Programs → Sessions tab.' },
      { type: 'body',  text: 'Backup: Wix Dashboard → Content Manager → Program Sessions' },
      { type: 'h1',   text: 'Fields' },
      ...
      { type: 'h1',   text: 'Steps (Staff)' },
      { type: 'body',  text: '1. Staff top nav → Programs → Sessions tab' },
      { type: 'body',  text: '2. Add session dates for the program (title, start/end, location)' },
      { type: 'body',  text: '3. Parents only see sessions for programs their students are enrolled in' },
      { type: 'h1',   text: 'Steps (CMS backup)' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'body',  text: '1. Sign in → Staff top nav → Messages.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'body',  text: '1. Sign in with an admin @shmspto.org account → Staff top nav → Members.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'body',  text: 'Recommended: Staff top nav → Minutes.' },
      { type: 'body',  text: 'Backup: Content Manager → Meeting Minutes' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'body',  text: 'SHMS PTO — Wix Admin Guide  |  Last updated: July 17, 2026' },
      { type: 'body',  text: 'Creating a product in Wix Stores is not enough — you must also add its Product ID to the store or spirit allowlist so the website knows which Catalog items belong on /store vs /spirit-wear. Board members can do both steps. No code deploy.' },
      { type: 'h1',   text: 'Why there is a list' },
      { type: 'body',  text: 'Wix Catalog holds every product (store snacks, spirit wear, memberships, store cards, demos). The site only displays products whose UUIDs are listed in Site Settings (editable from Staff → Store & spirit).' },
      ...
      { type: 'h1',   text: 'Recommended: Staff UUID lists' },
      { type: 'body',  text: 'After you have the Product ID, Staff top nav → Store & spirit — paste into the store or spirit list and save. Site Settings below remains the CMS backup.' },
      { type: 'h1',   text: 'Steps' },
      ...
      { type: 'body',  text: '☐ Correct Site Settings key updated (Staff → Store & spirit or CMS)' },
...

  {
    ...
    content: [
      ...
      { type: 'body', text: 'SHMS PTO — Staff Guide  |  Last updated: July 17, 2026' },
      { type: 'h1', text: 'What this controls' },
      { type: 'body', text: 'Surveys are hosted on the PTO site — parents never leave. The same branded link works in email, SMS, WhatsApp, and the member portal.' },
      { type: 'h1', text: 'Create in Staff (recommended)' },
      { type: 'body', text: '1. Sign in with @shmspto.org → Staff → Surveys.' },
      { type: 'body', text: '2. Click Create survey.' },
      { type: 'body', text: '3. Enter title (slug fills automatically), description, intro, and thank-you message.' },
      { type: 'body', text: '4. Add questions: short text, long text, email, multiple choice, or grade. Mark required as needed.' },
      { type: 'body', text: '5. Toggle Active, Show in member portal, and Require login as needed → Create survey.' },
      { type: 'h1', text: 'Edit an existing survey' },
      { type: 'body', text: 'Select the survey → Edit. Change questions or settings → Save changes. Uncheck Active to stop accepting responses without deleting history.' },
      { type: 'h1', text: 'Share it' },
      { type: 'body', text: 'Still on Staff → Surveys: choose the survey, then Open Email / SMS / WhatsApp, or Copy. Open live survey opens /survey/your-slug.' },
      { type: 'body', text: 'The channel is saved with every response so marketing can compare sources.' },
      { type: 'h1', text: 'Test before sending' },
      { type: 'body', text: 'Open the live link on phone and desktop, submit one response, then confirm it appears in the responses table (or Download CSV).' },
      { type: 'h1', text: 'Backup' },
      { type: 'body', text: 'Wix Content Manager → Surveys / Survey Responses remains the backup source of truth. You should not need to paste fieldsJson anymore.' },
    ],
  },
  {
    folder: 'wix',
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'body', text: 'Recommended board workflow: sign in to shmspto.org → Staff top nav → Social.' },
      ...
      { type: 'body', text: 'Facebook publish from Staff → Social is live. Instagram will unlock after connecting a second social account in Wix and setting socialInstagramAccountId.' },
    ],
  },

  {
    ...
    content: [
      ...
      { type: 'body', text: 'programs — enrichment + enrollee messaging + timesheet review' },
      ...
      { type: 'body', text: 'instructor — assigned programs: roster, schedule/flyer, messages, timesheets' },
      { type: 'body', text: 'coordinator — assigned programs: roster, messaging, timesheets (parent liaison)' },
      { type: 'h1', text: 'Assigned program IDs' },
      { type: 'body', text: 'For instructor and coordinator, paste program IDs (comma-separated) in Staff → Staff access. Copy IDs from Staff → Programs.' },
      { type: 'h1', text: 'Where they work' },
      { type: 'body', text: 'Staff has its own dark-green top nav (StaffShell) — not the public marketing nav. Open /staff after signing in with @shmspto.org. Parents still use /member-portal under a separate shell.' },
      { type: 'body', text: 'Workspaces (role-gated): Home, Projects, Members, Staff access, Social, Surveys, Messages, Minutes, Programs, Timesheets, Payments, Events, Store & spirit, Page copy, Expenses, Help.' },
      { type: 'body', text: 'Each role home shows what that position owns and a short This Week checklist.' },
      { type: 'body', text: 'Map of which role sees which workspace: doc 32. Year board / Projects: doc 29. Quick start: doc 30. Admin Members lookup: doc 31.' },
      { type: 'body', text: 'Also see: 02 Programs · 02b Timesheets · 02c Parent enrichment · 12b Payments · 13 Page copy · 15 Sessions · 16 Messages · 19 Minutes · 21 Store & spirit · 23–25 Surveys & Social.' },
      ...
      { type: 'body', text: 'Marketing, secretary, and admin can open or copy survey messages for email/SMS/WhatsApp, review responses, filter by survey, and download CSV. Docs 23–24.' },
      ...
      { type: 'h1', text: 'Year project board' },
      { type: 'body', text: 'Everyone on Staff sees Projects in the top nav — shared school-year swimlanes. Presidents and VPs add projects; assign tasks to people after you talk. Full walkthrough: doc 29.' },
      { type: 'h1', text: 'Admin lookup, act-as, archive' },
      { type: 'body', text: 'Member search, Act as, and Archive / Restore students under Staff → Members: doc 31.' },
      { type: 'h1', text: 'Staff quick start' },
      { type: 'body', text: 'New to /staff? Start with doc 30 — Staff Portal Quick Start.' },
      { type: 'h1', text: 'Legal pages' },
      { type: 'body', text: 'Public: /privacy /terms /photo-release — edit via PageContent legal-privacy, legal-terms, legal-photo-release (Staff → Page copy or CMS).' },
    ],
  },

  {
    ...
    content: [
      ...
      { type: 'body', text: 'SHMS PTO — Board Member Guide  |  Last updated: July 22, 2026' },
      ...
      { type: 'h1', text: 'Create account / sign in' },
      { type: 'body', text: 'New parents: /auth/join (Create Your Account). Existing: Log In on the same page. Google/Facebook available after they choose that option.' },
      ...
      { type: 'body', text: 'To add another child: Add a student. Enrichment registration also needs parent phone, emergency contact, and authorized pick-up on the student profile.' },
      { type: 'h1', text: 'Enrichment programs' },
      { type: 'body', text: 'Browse /programs for day, time, weeks, dates, price, and flyer. Register from the card. Free and paid members use the same flow. Waitlist confirmation appears if the class is full. Full walkthrough: doc 02c.' },
      ...
      { type: 'body', text: 'Admin: use Staff → Member lookup & act-as for troubleshooting. Treasurer: reconcile payment/load problems. Programs or instructor: roster / messages / waitlist. Marketing: manage survey links and social drafts.' },
...

  {
    ...
    content: [
      { type: 'title', text: 'Staff Year Project Board' },
      { type: 'body', text: 'SHMS PTO — Staff Guide  |  Last updated: July 17, 2026' },
      { type: 'body', text: 'Built into Staff (/staff) — no Asana, Trello, or other outside tools. Shared school-year projects with swimlanes, member assignment, and personal boards.' },
      { type: 'h1', text: 'Where to go' },
      { type: 'body', text: '1. Sign in with your @shmspto.org email (not your personal email).' },
      { type: 'body', text: '2. Open /staff — you get the dark-green Staff top nav.' },
      { type: 'body', text: '3. Staff top nav → Projects (Year board / My board / Project views).' },
      { type: 'h1', text: 'School year' },
      { type: 'body', text: 'The board uses the school year July → June (example: 2025-2026). Use the year dropdown if you need last year or next year.' },
      { type: 'h1', text: 'Three views' },
      { type: 'body', text: 'Year board — everyone on Staff can see every project for that year as a swimlane (column). Scroll sideways on a phone.' },
      { type: 'body', text: 'My board — tasks assigned to you, that you created, or on projects you lead.' },
      { type: 'body', text: 'Project — one project’s description, members, and tasks. Open a swimlane header or use the Project tab.' },
      { type: 'h1', text: 'Add a project (president / VP)' },
      { type: 'body', text: '1. Expand Add a project.' },
      { type: 'body', text: '2. Name it clearly (Fall Festival, Spirit Wear drop, Teacher Appreciation week).' },
      { type: 'body', text: '3. Choose your lead role and check the members who will help.' },
      { type: 'body', text: '4. Add project — it becomes a new swimlane on the Year board.' },
      { type: 'h1', text: 'Add and assign a task' },
      { type: 'body', text: '1. Under Add a task, pick the project and write what needs to happen.' },
      { type: 'body', text: '2. Talk with the person first (chat, text, meeting) — then Assign them from the dropdown.' },
      { type: 'body', text: '3. Set a due date when you can.' },
      { type: 'body', text: '4. The same task now shows on: the Year board swimlane, that Project view, and their My board.' },
      { type: 'body', text: 'You can also leave Assign later and assign from the task card after you talk.' },
      { type: 'h1', text: 'Who can do what' },
      { type: 'body', text: 'Everyone — see the Year board.' },
      { type: 'body', text: 'Project members — add tasks and assign people on that project.' },
      { type: 'body', text: 'Project lead (or admin) — edit members, mark the project done.' },
      { type: 'body', text: 'Assignee — mark their task Done (or Unblock if it was waiting on something).' },
      { type: 'h1', text: 'Tips' },
      { type: 'body', text: 'One project per real initiative — not one giant “Marketing” bucket for the whole year.' },
      { type: 'body', text: 'Check Show done when you need history.' },
      { type: 'body', text: 'If someone is missing from Assign, they need Staff access first (doc 26) or add them as a project member.' },
      { type: 'h1', text: 'What it is not' },
      { type: 'body', text: 'Not Gantt charts, sprints, or time tracking — just shared year swimlanes plus assigned work so the board stays aligned without another app.' },
    ],
  },

  {
    folder: 'wix',
    update: true,
    title: '30 - Staff Portal Quick Start',
    content: [
      { type: 'title', text: 'Staff Portal Quick Start' },
      { type: 'body', text: 'SHMS PTO — Staff Guide  |  Last updated: July 17, 2026' },
      { type: 'body', text: 'This is the START HERE guide for anyone using /staff. Staging: https://shmspto.vercel.app/staff  |  After DNS: https://www.shmspto.org/staff' },
      { type: 'h1', text: 'Three separate shells' },
      { type: 'body', text: 'Visitor — public marketing nav on the site.' },
      { type: 'body', text: 'Free / paid member — MemberShell on /member-portal (personal email; students & store cards).' },
      { type: 'body', text: 'Staff — dark-green StaffShell on /staff (@shmspto.org + StaffRoles). Completely separate chrome from the public and parent shells.' },
      { type: 'h1', text: 'Sign in the right way' },
      { type: 'body', text: 'Staff work uses your @shmspto.org email only.' },
      { type: 'body', text: 'Your personal email is for the parent portal (your own kids, store cards). Keep those separate — details in doc 26.' },
      { type: 'body', text: 'First time: sign in with @shmspto.org → open /staff once. That registers you. An admin then checks your roles under Staff access.' },
      { type: 'h1', text: 'Staff top nav workspaces' },
      { type: 'body', text: 'You only see workspaces your roles unlock (doc 32):' },
      { type: 'body', text: 'Home — role home cards + This week checklist' },
      { type: 'body', text: 'Projects — year swimlanes / My board (doc 29)' },
      { type: 'body', text: 'Members — admin lookup, act-as, archive students (doc 31)' },
      { type: 'body', text: 'Staff access — assign @shmspto.org roles (admin)' },
      { type: 'body', text: 'Social — Facebook compose (doc 25)' },
      { type: 'body', text: 'Surveys — create, share, review, CSV (docs 23–24)' },
      { type: 'body', text: 'Messages — parent inbox (doc 16)' },
      { type: 'body', text: 'Minutes — meeting minutes (doc 19)' },
      { type: 'body', text: 'Programs — registration / featured + Sessions tab (docs 02, 15)' },
      { type: 'body', text: 'Payments — Needs Reconciliation (doc 12b)' },
      { type: 'body', text: 'Events — upcoming list + Manage in Wix Events (doc 07)' },
      { type: 'body', text: 'Store & spirit — product UUID lists (doc 21)' },
      { type: 'body', text: 'Page copy — heroes and marketing text (doc 13)' },
      { type: 'body', text: 'Help — in-app pointers to these docs' },
      { type: 'h1', text: 'Which doc to open' },
      { type: 'body', text: '26 - Staff Roles & Portal Workspaces — logins, roles, shells' },
      { type: 'body', text: '32 - Staff Workspaces Map (Role Tools) — who sees which nav item' },
      { type: 'body', text: '29 - Staff Year Project Board — projects, swimlanes, assign tasks' },
      { type: 'body', text: '31 - Admin Lookup, Act-as & Student Archive — help a parent / archive a student' },
      { type: 'body', text: '27 - Member Portal Parent Support Guide — walk a parent through their portal' },
      { type: 'body', text: '12b · 13 · 15 · 16 · 19 · 21 · 23–25 — Payments, Page copy, Sessions, Messages, Minutes, Store & spirit, Surveys & Social' },
      { type: 'h1', text: 'Common first-week checklist' },
      { type: 'body', text: '1. Confirm you can open /staff with @shmspto.org and your roles look right.' },
      { type: 'body', text: '2. Add or join at least one project under Projects for something you own.' },
      { type: 'body', text: '3. Assign one real task to a teammate after you talk with them — confirm it shows on their My board.' },
      { type: 'body', text: '4. If you are Marketing: draft (do not publish yet) a Facebook post from Staff → Social.' },
      { type: 'body', text: '5. If you are Programs/instructor: send a test message from Staff → Messages.' },
      { type: 'h1', text: 'Need help from Rob' },
      { type: 'body', text: 'Code deploys, Vercel env vars, DNS cutover, Square/Wix OAuth — docs 09, 10, 22, 28. Everything else on this list is for the board in Staff or Wix CMS.' },
    ],
  },

  {
    folder: 'wix',
    update: true,
    title: '31 - Admin Lookup, Act-as & Student Archive',
    content: [
      { type: 'title', text: 'Admin Lookup, Act-as & Student Archive' },
      { type: 'body', text: 'SHMS PTO — Staff Guide  |  Last updated: July 17, 2026' },
      { type: 'body', text: 'Admin (president) tools on /staff for helping parents without asking them for passwords or card numbers.' },
      { type: 'h1', text: 'Who can use this' },
      { type: 'body', text: 'Only staff with the admin role. Other roles will not see Members in the Staff top nav.' },
      { type: 'h1', text: 'Member lookup' },
      { type: 'body', text: '1. Staff top nav → Members.' },
      { type: 'body', text: '2. Search by parent email or student name.' },
      { type: 'body', text: '3. Results show the parent email and their students (grade + Archived if hidden).' },
      { type: 'h1', text: 'Act as' },
      { type: 'body', text: '1. Click Act as next to a parent.' },
      { type: 'body', text: '2. You open /member-portal as that parent sees it — useful for “I can’t find my store card” or “where is my message?”' },
      { type: 'body', text: '3. Use Exit act-as (banner) to return. Your staff session stays yours; you are not permanently logged in as them.' },
      { type: 'body', text: 'Do not use act-as to change passwords or enter payment card numbers for a parent.' },
      { type: 'h1', text: 'Archive a student' },
      { type: 'body', text: 'Use when a student left the school or should no longer appear in the parent portal, but you need history kept.' },
      { type: 'body', text: '1. Find the parent in lookup.' },
      { type: 'body', text: '2. Next to the student, click Archive and confirm.' },
      { type: 'body', text: '3. The student is hidden from the parent portal. Payments and store-card history stay in the system.' },
      { type: 'body', text: '4. Archive also stops auto top-off and blocks new reloads for that student.' },
      { type: 'body', text: '5. Click Restore later if they return.' },
      { type: 'h1', text: 'Related parent help' },
      { type: 'body', text: 'Walk a parent through their own portal screens: doc 27. Store card reloads and Needs Reconciliation: doc 12b.' },
    ],
  },

  {
    ...
    content: [
      ...
      { type: 'body', text: 'SHMS PTO — Staff Guide  |  Last updated: July 17, 2026' },
      { type: 'body', text: 'Staff top nav shows only the workspaces for your roles.' },
      { type: 'body', text: 'Staging: https://shmspto.vercel.app/staff  |  Full role overview: doc 26 · Quick start: doc 30' },
      { type: 'h1', text: 'Home' },
      { type: 'body', text: 'Everyone on Staff. Role home cards + This Week checklist. Doc 26, 30.' },
      { type: 'h1', text: 'Projects' },
      { type: 'body', text: 'Everyone on Staff. Year board / My board / Project swimlanes. Doc 29.' },
      { type: 'h1', text: 'Members' },
      { type: 'body', text: 'admin. Lookup, Act as, Archive / Restore students. Doc 31.' },
      { type: 'h1', text: 'Staff access' },
      { type: 'body', text: 'admin. Assign @shmspto.org roles. Doc 26.' },
      { type: 'h1', text: 'Social' },
      { type: 'body', text: 'marketing, admin. Facebook compose (Instagram when connected). Doc 25.' },
      { type: 'h1', text: 'Surveys' },
      { type: 'body', text: 'marketing, secretary, admin. Create, share, review, CSV. Docs 23–24.' },
      { type: 'h1', text: 'Messages' },
      { type: 'body', text: 'programs, instructor, secretary, admin. Parent portal inbox. Doc 16.' },
      { type: 'h1', text: 'Minutes' },
      { type: 'body', text: 'secretary, admin. Meeting minutes publish. Doc 19.' },
      { type: 'h1', text: 'Programs' },
      { type: 'body', text: 'programs, instructor, coordinator, admin. Schedule, flyer, roster, sessions, message class. Docs 02, 15.' },
      { type: 'h1', text: 'Timesheets' },
      { type: 'body', text: 'instructor, coordinator (submit); programs, admin (approve). Doc 02b.' },
      { type: 'h1', text: 'Payments' },
      { type: 'body', text: 'treasurer, admin. Needs Reconciliation — Retry gift-card load / Mark reconciled. Doc 12b.' },
      { type: 'h1', text: 'Events' },
      { type: 'body', text: 'events, secretary, marketing, admin. Create/edit events + upload flyer. Doc 07.' },
      { type: 'h1', text: 'Store & spirit' },
      { type: 'body', text: 'retail, admin. Product UUID allowlists. Doc 21.' },
      { type: 'h1', text: 'Page copy' },
      { type: 'body', text: 'marketing, secretary, admin. Heroes, marketing copy, optional page flyer upload. Doc 13.' },
      { type: 'h1', text: 'Expenses' },
      { type: 'body', text: 'Any staff submit; admin approves; treasurer marks paid.' },
      { type: 'h1', text: 'Help' },
      { type: 'body', text: 'Everyone on Staff. Pointers into these Drive docs.' },
    ],
  },
  ...
  // ── DNS cutover ───────────────────────────────────────────────────────────
  {
    folder: 'techops',
    update: true,
    title: '28 - DNS Cutover & Post-DNS Update Checklist (Rob)',
    content: [
      { type: 'title', text: 'DNS Cutover & Post-DNS Update Checklist' },
      { type: 'body', text: 'SHMS PTO Platform — Tech Ops Reference  |  Last updated: July 16, 2026' },
      { type: 'body', text: 'Canonical after cutover: https://www.shmspto.org' },
      { type: 'body', text: 'Staging now: https://shmspto.vercel.app' },
      { type: 'body', text: 'Do DNS last — after page-by-page visitor QA on staging. CheddarUp webhook and Instagram remain backlog until you choose otherwise.' },

      { type: 'h1', text: 'Before you change DNS' },
      { type: 'body', text: '1. Finish page-by-page public visitor QA on https://shmspto.vercel.app' },
      { type: 'body', text: '2. Finish logged-in QA (portal, membership, store card, spirit wear)' },
      { type: 'body', text: '3. Square webhook already on staging: https://shmspto.vercel.app/api/webhooks/square — event gift_card.activity.created — signature key matches Vercel SQUARE_WEBHOOK_SIGNATURE_KEY' },

      { type: 'h1', text: 'A. DNS change (you)' },
      { type: 'body', text: 'At the DNS host for shmspto.org (currently Wix DNS):' },
      { type: 'body', text: 'A record: @ → 76.76.21.21' },
      { type: 'body', text: 'CNAME: www → cname.vercel-dns.com' },
      { type: 'body', text: 'In Vercel → Project frontend → Settings → Domains: confirm www.shmspto.org and shmspto.org are attached; complete any TXT verification; keep www as primary (apex → www redirect is already in vercel.json).' },
      { type: 'body', text: 'Ready when: dig www.shmspto.org no longer shows *.wixdns.net and the site loads from Vercel.' },
      { type: 'body', text: 'Then tell the agent: DNS is live — so the updates below can be applied in one pass.' },

      { type: 'h1', text: 'B. Post-DNS updates — full list' },
      { type: 'body', text: 'Everything that still points at staging must move to https://www.shmspto.org.' },

      { type: 'h2', text: 'B1. Vercel Production env → then redeploy (agent or you)' },
      { type: 'body', text: 'NEXT_PUBLIC_SITE_URL = https://www.shmspto.org' },
      { type: 'body', text: 'SQUARE_NOTIFICATION_URL = https://www.shmspto.org/api/webhooks/square' },
      { type: 'body', text: 'Also update frontend/.env.local the same way (not committed). Redeploy frontend after saving so OAuth, checkout returns, and Square signature hashing use www.' },

      { type: 'h2', text: 'B2. Square Developer Dashboard (you)' },
      { type: 'body', text: 'Webhook subscription URL → https://www.shmspto.org/api/webhooks/square' },
      { type: 'body', text: 'Event stays: gift_card.activity.created' },
      { type: 'body', text: 'Keep the existing signature key, or rotate it and update Vercel SQUARE_WEBHOOK_SIGNATURE_KEY.' },
      { type: 'body', text: 'Must match SQUARE_NOTIFICATION_URL character-for-character (signature = HMAC of URL + body).' },

      { type: 'h2', text: 'B3. CheddarUp / Zapier (you — backlog OK)' },
      { type: 'body', text: 'Webhook POST URL → https://www.shmspto.org/api/webhooks/cheddarup?token=<CHEDDARUP_WEBHOOK_SECRET>' },
      { type: 'body', text: 'Secret already lives in Vercel as CHEDDARUP_WEBHOOK_SECRET.' },

      { type: 'h2', text: 'B4. Wix OAuth app (you)' },
      { type: 'body', text: 'Wix Dashboard → OAuth / Headless app (SHMS PTO Web) — Allowed redirect URIs must include:' },
      { type: 'body', text: 'https://www.shmspto.org/auth/callback' },
      { type: 'body', text: 'https://shmspto.org/auth/callback (apex, if anyone hits it)' },
      { type: 'body', text: 'Keep https://shmspto.vercel.app/auth/callback until www is confirmed stable.' },

      { type: 'h2', text: 'B5. Wix CMS SiteSettings (agent or you)' },
      { type: 'body', text: 'membershipOrdersWebhookUrl → https://www.shmspto.org/api/webhooks/wix-orders?token=<WIX_ORDERS_WEBHOOK_SECRET>' },
      { type: 'body', text: 'spiritWearBaseUrl → https://www.shmspto.org/store/product-page (or the live product-page root you settle on)' },
      { type: 'body', text: 'Footer / social URLs (socialFacebook, etc.) already use absolute Meta URLs — no DNS change needed.' },

      { type: 'h2', text: 'B6. GitHub Actions (agent)' },
      { type: 'body', text: 'File: .github/workflows/sync-membership-orders.yml' },
      { type: 'body', text: 'SYNC_URL → https://www.shmspto.org/api/cron/sync-membership-orders' },
      { type: 'body', text: 'CRON_SECRET GitHub secret stays as-is.' },

      { type: 'h2', text: 'B7. Code fallbacks (agent — optional cleanup after env is set)' },
      { type: 'body', text: 'These hardcode staging as a fallback when env is missing. Cutover day does not require code changes if Vercel env is updated and redeployed. Update later so a missing env does not send people to staging:' },
      { type: 'body', text: 'frontend/lib/wix-oauth-client.ts — fallback callback host' },
      { type: 'body', text: 'frontend/lib/wix-ecom-checkout.ts — checkout return URLs' },
      { type: 'body', text: 'frontend/lib/surveys/parse.ts — survey share base URL' },
      { type: 'body', text: 'scripts/smoke-production.mjs — default smoke base URL' },
      { type: 'body', text: 'frontend/.env.example — example staging URLs' },

      { type: 'h2', text: 'B8. Docs cleanup (agent — after cutover)' },
      { type: 'body', text: 'Update QA test plan base URL to www; refresh Drive docs that still say staging is primary; keep shmspto.vercel.app listed as staging alias only.' },

      { type: 'h2', text: 'B9. Optional cleanup' },
      { type: 'body', text: 'Soft-land classic Wix site (no longer serving www). Confirm Vercel TLS is green for www + apex. Keep shmspto.vercel.app as a staging alias for emergency / smoke tests.' },

      { type: 'h1', text: 'C. Final production URL reference' },
      { type: 'body', text: 'Site — https://www.shmspto.org' },
      { type: 'body', text: 'OAuth callback — https://www.shmspto.org/auth/callback' },
      { type: 'body', text: 'Square webhook — https://www.shmspto.org/api/webhooks/square' },
      { type: 'body', text: 'CheddarUp webhook — https://www.shmspto.org/api/webhooks/cheddarup?token=<CHEDDARUP_WEBHOOK_SECRET>' },
      { type: 'body', text: 'Wix orders webhook — https://www.shmspto.org/api/webhooks/wix-orders?token=<WIX_ORDERS_WEBHOOK_SECRET>' },
      { type: 'body', text: 'Membership sync cron — https://www.shmspto.org/api/cron/sync-membership-orders' },

      { type: 'h1', text: 'D. Who does what' },
      { type: 'body', text: '1. Flip DNS A/CNAME — You' },
      { type: 'body', text: '2. Vercel NEXT_PUBLIC_SITE_URL + SQUARE_NOTIFICATION_URL + redeploy — Agent (or you)' },
      { type: 'body', text: '3. Square webhook URL → www — You' },
      { type: 'body', text: '4. Wix OAuth redirect URIs → www — You' },
      { type: 'body', text: '5. CMS membershipOrdersWebhookUrl + spiritWearBaseUrl — Agent (or you)' },
      { type: 'body', text: '6. GHA SYNC_URL → www — Agent' },
      { type: 'body', text: '7. CheddarUp webhook → www — You (backlog OK)' },
      { type: 'body', text: '8. Code fallback cleanup — Agent (later)' },
      { type: 'body', text: '9. Re-run QA on www — You + agent smoke' },

      { type: 'h1', text: 'E. Post-DNS QA checklist' },
      { type: 'body', text: 'Re-run against https://www.shmspto.org:' },
      { type: 'body', text: 'Home + all nav pages load' },
      { type: 'body', text: 'Log in / Sign up → callback on www' },
      { type: 'body', text: 'Member portal' },
      { type: 'body', text: 'Membership Join → checkout → return to www' },
      { type: 'body', text: 'Store card load / card-on-file' },
      { type: 'body', text: 'Spirit wear Buy' },
      { type: 'body', text: 'Contact + newsletter' },
      { type: 'body', text: 'Square webhook still verifies (auto top-off path if testing)' },
      { type: 'body', text: 'CheddarUp test payment (when not backlog)' },
      { type: 'body', text: 'GHA membership sync still OK (workflow_dispatch once)' },

      { type: 'h1', text: 'F. Staging vs production' },
      { type: 'body', text: 'Public site: staging = shmspto.vercel.app → after DNS = www.shmspto.org' },
      { type: 'body', text: 'Square webhook: staging path → same path on www' },
      { type: 'body', text: 'GHA sync: staging cron URL → www cron URL' },
      { type: 'body', text: 'OAuth: staging callback → www callback (keep staging temporarily)' },
      { type: 'body', text: 'CheddarUp: backlog → www when ready' },
      { type: 'body', text: 'Instagram: backlog' },
      { type: 'body', text: 'Facebook page link: no DNS change (facebook.com/stonehillmspto)' },
    ],
  },
  ...
  // ── Index doc ─────────────────────────────────────────────────────────────
  {
    ...
    content: [
      ...
      { type: 'body',  text: 'Last updated: July 22, 2026  |  Questions? Email gregory.robert.c@gmail.com' },
      { type: 'body',  text: 'Live site: https://www.shmspto.org' },
      { type: 'h1',   text: 'Staff portal (/staff) — start here' },
      { type: 'body',  text: '30 - Staff Portal Quick Start (read this first)' },
      { type: 'body',  text: '26 - Staff Roles & Portal Workspaces (@shmspto.org login, roles, dark-green StaffShell)' },
      { type: 'body',  text: '32 - Staff Workspaces Map (Role Tools) — who sees which top-nav item' },
      { type: 'body',  text: '29 - Staff Year Project Board (Staff → Projects)' },
      { type: 'body',  text: '31 - Admin Lookup, Act-as & Student Archive (Staff → Members)' },
      { type: 'body',  text: '02 - Programs (schedule, flyer, roster, waitlist) · 02b Timesheets · 15 Sessions · 16 Messages' },
      { type: 'body',  text: 'Staff tools also covered in: 12b Payments · 13 Page copy · 19 Minutes · 21 Store & spirit · 23–25 Surveys & Social' },
      { type: 'body',  text: '27 - Member Portal Parent Support Guide · 02c Parent Enrichment Registration' },
      { type: 'h1',   text: 'Wix Admin Guides (board members)' },
      { type: 'body',  text: '01–08 — Board, Programs, Membership tiers, Announcement, Fundraising, FAQ, Events, Spirit Wear / Store inventory (use latest v2 titles; Staff paths noted where shipped)' },
      { type: 'body',  text: '12b - How to Manage the Store Card (includes Staff → Payments reconciliation)' },
      { type: 'body',  text: '13 - How to Edit Page Heroes and Marketing Copy (Staff → Page copy; CMS backup)' },
      { type: 'body',  text: '14 - How to Edit Member Portal Copy' },
      { type: 'body',  text: '15 - How to Add Program Session Dates (Staff → Programs → Sessions; CMS backup)' },
      ...
      { type: 'body',  text: 'Staff on /staff: docs 26, 29–32, plus tools in 12b / 13 / 15 / 16 / 19 / 21 / 23–25' },
      { type: 'body',  text: 'Board with Wix access: docs 01–08, 12b, 13–21 (CMS backup paths)' },
      ...
      { type: 'body',  text: 'If it is board day-to-day work → /staff (docs 30 / 26 / 32 / 29). If it is words, links, dates, products, or CMS rows → Staff workspace when available, else Wix. If it needs Cursor or the Vercel Dashboard → docs 09 / 22.' },
    ],
  },
]
