...
const DOCS = [
...

  {
    ...
    content: [
      ...
      { type: 'body', text: 'Base link: https://shmspto.org/survey/YOUR-SLUG' },
      { type: 'body', text: 'Email: add ?from=email  |  SMS: ?from=sms  |  WhatsApp: ?from=whatsapp' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'body', text: 'SHMS PTO — Wix Admin Guide  |  Last updated: July 2026' },
      { type: 'h1', text: 'Where to go' },
      { type: 'body', text: 'Wix Dashboard → Content Manager → Survey Responses' },
      ...
      { type: 'body', text: '1. Filter surveySlug to the survey you want.' },
      { type: 'body', text: '2. Sort submittedAt newest first.' },
      { type: 'body', text: '3. Use channel to compare which outreach worked.' },
      { type: 'h1', text: 'Export for analysis' },
      { type: 'body', text: 'Use the collection More Actions / Export option to download CSV. Keep the original collection as the source of truth.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'body', text: 'Create the SHMS PTO Facebook Page and Instagram professional account. Connect Instagram to the Facebook Page in Meta.' },
      { type: 'h1', text: 'Connect in Wix' },
      { type: 'body', text: 'Wix Dashboard → Marketing & SEO → Social → Connect Accounts. Sign in to Meta and approve the SHMS PTO Page and Instagram account.' },
      { type: 'body', text: 'After connection, add the public profile URLs in Content Manager → Site Settings: socialFacebook and socialInstagram.' },
      ...
      { type: 'body', text: '1. Choose Facebook, Instagram, or both.' },
      { type: 'body', text: '2. Add post text, an optional square image URL (1080×1080 recommended), and an optional link.' },
      { type: 'body', text: '3. Save draft while publishing is off. After Wix Social is connected and socialPublishEnabled=true, use Publish.' },
      ...
      { type: 'body', text: 'Code support is ready, but publishing remains disabled until the accounts are connected and socialPublishEnabled is set to true in Site Settings.' },
    ],
  },

  {
    ...
    content: [
      ...
      { type: 'h1', text: 'Assign a staff login' },
      { type: 'body', text: '1. Person needs a Wix member account under their @shmspto.org email (sign up on shmspto.org with that address).' },
      { type: 'body', text: '2. Content Manager → StaffRoles → New Item.' },
      { type: 'body', text: '3. email = the @shmspto.org login (lowercase). roles = comma-separated system roles. boardTitle = public office. active = true.' },
      ...
      { type: 'body', text: 'Choose Facebook and/or Instagram, write the post, add optional image/link URLs, then Save draft or Publish when live publishing is enabled.' },
