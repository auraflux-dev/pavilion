...
| [auraflux-dev/pavilion](https://github.com/auraflux-dev/pavilion) | Pavilion platform + marketing | onpavilion.com, demo, trials |
...
**Never** create one-off `pavilion-*` / ship scratch projects. Deploy with CLI into the allowlisted project id, from a clean worktree, no dirty tree.
...
**Pavilion:** deploy from `pavilion` to robert-4220 projects (`commons-site`, `commons-pto-demo`, `commons-pto`). Never deploy monorepo `frontend/` to treasurer or to a robert-4220 project named `frontend`.
...
- **Drift check:** `node scripts/shms-frontend-parity.mjs` (compares sibling `~/pavilion` and `~/shmspto` trees). Fail means school-facing files exist only in the monorepo and will not ship to www.
