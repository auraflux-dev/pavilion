/**
 * Product brand shell for commons-site (Vercel project / infra name).
 * Product name: Pavilion. Colors: forest green + gold (product tokens).
 * Layout patterns follow Business Rocket marketing; brand stays Pavilion.
 * Legal seller is always HSKRG LLC (Stripe).
 * Marketing domain: onpavilion.com.
 */
export const PRODUCT_NAME = 'Pavilion'
export const LEGAL_ENTITY = 'HSKRG LLC'
export const PRODUCT_TAGLINE = 'The PTO operating system.'
export const CONTACT_EMAIL = 'hello@hskrg.com'
export const PRODUCT_DOMAIN = 'onpavilion.com'

/** Locked color direction: forest green + gold. */
export const BRAND_DIRECTION_ID = 'forest-gold'
