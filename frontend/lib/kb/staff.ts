...
Every morning at **6:00am Eastern during daylight time** (5:00am during standard time) president@ gets an email of **yesterday** (Eastern midnight-midnight): website forms and surveys, payments and event tickets, member portal students / memberships / enrollments / household invites / help / portal messages, **auth** (password resets requested, reset-link opens, logins ok/failed, logouts), staff tasks / minutes / outreach / audit, president inbox mail, and logged errors. Every report also includes **weekly traffic** (last 7 Eastern days ending yesterday): website vs member portal vs staff pageviews, unique-browser days, and top paths. Extra recipients: **Staff → Site settings → Contact → Daily activity report extra emails**.

## Platform Activity (auth log)
Open **Staff → Activity** (admins, membership, and secretary). Filter the last 48 hours or 7 days. Rows show password-reset requests, reset-link opens, login success/fail (email or Google), and logout. Identities are **hashed** (domain + short hash). Passwords and raw reset tokens are never stored. Use this when a parent says “I reset my password” and you need to confirm the attempt without waiting for tomorrow’s email.
