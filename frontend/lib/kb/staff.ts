...
Change one list at a time, save, then **View site** to confirm. Leave deep Wix Editor work to whoever owns theme-level changes.`,
    },
    {
      ...
      order: 3,
      body: `Successful membership, Cove load, ticket, donation, and shop purchases create a parent portal message (and email when Gmail send is connected). The same checkout also emails **vp-membershipexperience@shmspto.org**, **treasurer@shmspto.org**, and **cove@shmspto.org** (Cove / store coordinator) a staff sale alert (includes shirt size / magnet / refreshments notes when membership).
